...
const getNonReccurTasks = async () => {
  const res = await prisma.nonReccurTask.findMany();
  return res;
}

const Task = async () => {
  const tasks = await getNonReccurTasks();
  console.log(tasks);
...
=======
                    ...
                    <table className='container table-auto max-w-screen bg-green-300 rounded-md p-4'>
                      ...
                      <tbody className='bg-gray-100 overflow-auto'>
                        {tasks.map((Task) =>(
                        <tr key={Task.id}className='*:p-4 justify-between'>
                          <td>{Task.task}</td>
                          <td>Web-based solutions</td>
                          <td>IP Solutions</td>
                          <td>Planning/Design</td>
